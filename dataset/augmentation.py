...
class MotionAugmentation:
    ...
    def __call__(self, fgrs, phas, bgrs):
        ...
        fgrs = F.resized_crop(fgrs, *params, (self.size, self.size), interpolation=F.InterpolationMode.BILINEAR)
    ...
    def affine_all_with_bgr_phas(self, fgrs, phas, bgrs):
        # Foreground affine
        if random.random() < self.prob_fgr_affine:
            fgrs, phas = self._motion_affine(fgrs, phas)

        bgr_phas = torch.cat([torch.zeros((1, *phas.shape[1:])), phas[:-1]], dim=0)
        # Background affine
        if random.random() < self.prob_bgr_affine / 2:
            bgrs = self._motion_affine(bgrs)
            # bgrs, bgr_phas = self._motion_affine(bgrs, bgr_phas)
        if random.random() < self.prob_bgr_affine / 2:
            fgrs, phas, bgrs, bgr_phas = self._motion_affine(fgrs, phas, bgrs, bgr_phas)
                
        # Still Affine
        if self.static_affine:
            fgrs, phas, bgr_phas = self._static_affine(fgrs, phas, bgr_phas, scale_ranges=(0.5, 1))
            bgrs = self._static_affine(bgrs, scale_ranges=(1, 1.5))
        # Pause
        if random.random() < self.prob_pause:
            fgrs, phas, bgrs, bgr_phas = self._motion_pause(fgrs, phas, bgrs, bgr_phas)
        return fgrs, phas, bgrs, bgr_phas
    
    def bgr_augmentation(self, bgrs):
        ...
        # Pause
        if random.random() < self.prob_pause:
            bgrs = self._motion_pause(bgrs)
        
        return bgrs

    def _static_affine(self, *imgs, scale_ranges):
        _ = imgs[0][0]
        size = _.shape[-2:] if isinstance(_, torch.Tensor) else _.size
        params = transforms.RandomAffine.get_params(
            degrees=(-10, 10), translate=(0.1, 0.1), scale_ranges=scale_ranges,
            shears=(-5, 5), img_size=size)
        for t in range(len(imgs[0])):
            for img in imgs:
                img[t] = F.affine(img[t], *params, F.InterpolationMode.BILINEAR)
        # imgs = [
        #     [F.affine(t, *params, F.InterpolationMode.BILINEAR) for t in img]
        #     for img in imgs
        # ]
    ...
    def _motion_affine(self, *imgs):
        ...
        config = dict(degrees=(-10, 10), translate=(0.1, 0.1),
                      scale_ranges=(0.9, 1.1), shears=(-5, 5), img_size=size)
...
class TrainFrameSamplerAddFarFrame(TrainFrameSampler):
    ...
    def __call__(self, seq_length):
        ...
        dt = frames[0]-frames[1]
        frames = [frames[0] + dt * random.choice(self.far_mult)] + frames
        return frames
